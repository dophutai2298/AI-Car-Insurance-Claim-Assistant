  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const claim = await createClaim.mutateAsync({
        ...
        incident: {
          occurred_at: new Date(incidentAt).toISOString(),
      ...
      <Card className="rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <Card.Content className="p-6">
          <form className="grid gap-5" onSubmit={handleSubmit}>
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div className="grid gap-2">
                <Autocomplete
                  ...
                  value={make || null}
                  onChange={(key) => setMake(key ? String(key) : "")}
                >
                  ...
                  <Autocomplete.Popover>
                    <Autocomplete.Filter filter={contains}>
                      ...
                      {!vehicleMakes.isPending && !vehicleMakes.isError ? (
                        ...
                        >
                          {(vehicleMakes.data ?? []).map((manufacturer) => (
                            <ListBox.Item
                              id={manufacturer.name}
                              key={manufacturer.id}
                              textValue={manufacturer.name}
                            >
                              {manufacturer.name}
                ...
                {vehicleMakes.isPending ? (
                  <p className="text-sm text-slate-500" role="status">
                    {t("claim.loadingMakes")}
                  </p>
                ) : null}
                {vehicleMakes.isError ? (
                  <p className="text-sm text-red-700" role="alert">
                    {t("claim.unableToLoadMakes")}
                  </p>
                ) : null}
                {!vehicleMakes.isPending &&
                !vehicleMakes.isError &&
                (vehicleMakes.data?.length ?? 0) === 0 ? (
                  <p className="text-sm text-slate-500">{t("claim.noMakes")}</p>
                ) : null}
              </div>
            ...
            <TextField fullWidth isRequired name="incident_description">
