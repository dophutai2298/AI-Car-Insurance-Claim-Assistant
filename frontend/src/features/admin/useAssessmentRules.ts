...
import { getAssessmentRuleHistory, getAssessmentRules, updateAssessmentRules } from './adminApi'
...
export function useUpdateAssessmentRules() {
  ...
  return useMutation({
    mutationFn: (values: AssessmentRuleValues) => updateAssessmentRules(values, session!.access_token),
    onSuccess: async () => {
