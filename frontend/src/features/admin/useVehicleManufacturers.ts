...
import type { VehicleManufacturerUpdate, VehicleManufacturerWrite } from '../vehicleMakes/types'
import {
...
function invalidateVehicleManufacturerQueries(queryClient: ReturnType<typeof useQueryClient>) {
  return Promise.all([
...
export function useCreateVehicleManufacturer() {
  ...
  return useMutation({
    mutationFn: (values: VehicleManufacturerWrite) => createVehicleManufacturer(values, session!.access_token),
...
export function useUpdateVehicleManufacturer() {
  ...
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ manufacturerId, values }: { manufacturerId: number; values: VehicleManufacturerUpdate }) =>
